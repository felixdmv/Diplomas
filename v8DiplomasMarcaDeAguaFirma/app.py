...
class AppUnificada:
    ...
    def construir_tab_enviar(self):
        ...
        f_metodo = ttk.LabelFrame(frame, text=" 1. Elige cómo enviar los correos ", padding=15)
        f_metodo.pack(fill=tk.X, pady=5)
        
        self.var_metodo_envio = tk.StringVar(value="outlook") # Valor por defecto
        
        # Opciones con RadioButton
        ttk.Radiobutton(f_metodo, text="Usar la aplicación Outlook de mi PC (Recomendado si usas Outlook)", 
                        variable=self.var_metodo_envio, value="outlook", command=self.actualizar_ui_envio).pack(anchor="w")
        ttk.Radiobutton(f_metodo, text="Usar mi cuenta de Gmail / Google Workspace", 
                        variable=self.var_metodo_envio, value="gmail", command=self.actualizar_ui_envio).pack(anchor="w")
        ttk.Radiobutton(f_metodo, text="Usar mi cuenta de Microsoft 365 / Hotmail / Outlook.com", 
                        variable=self.var_metodo_envio, value="office365", command=self.actualizar_ui_envio).pack(anchor="w")
        ttk.Radiobutton(f_metodo, text="Otro (Configuración SMTP manual)", 
                        variable=self.var_metodo_envio, value="manual", command=self.actualizar_ui_envio).pack(anchor="w")

        ttk.Separator(frame, orient="horizontal").pack(fill=tk.X, pady=20)

        # --- CONTENEDOR DE DATOS (que cambia dinámicamente) ---
        self.f_datos_envio = ttk.LabelFrame(frame, text=" 2. Completa los datos ", padding=15)
        self.f_datos_envio.pack(fill=tk.X, pady=5)
        
        # -- Campos Comunes (Excel y Carpeta) --
        ttk.Label(self.f_datos_envio, text="Excel de Alumnos:").pack(anchor="w")
        f_ex = ttk.Frame(self.f_datos_envio, style="White.TFrame")
        f_ex.pack(fill=tk.X, pady=(2, 10))
        ...
        ttk.Label(self.f_datos_envio, text="Carpeta con PDFs Firmados:").pack(anchor="w")
        f_dir = ttk.Frame(self.f_datos_envio, style="White.TFrame")
        f_dir.pack(fill=tk.X, pady=(2, 10))
        self.ent_dir_pdfs = ttk.Entry(f_dir)
        self.ent_dir_pdfs.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 10))
        ttk.Button(f_dir, text="Seleccionar", command=lambda: self.sel_carpeta(self.ent_dir_pdfs)).pack(side=tk.LEFT)
        
        # -- Contenedor para campos SMTP (Email, Pass, Servidor, Puerto) --
        self.f_smtp_fields = ttk.Frame(self.f_datos_envio, style="White.TFrame")
        self.f_smtp_fields.pack(fill=tk.X, pady=10)
        # (Aquí se crearán los campos dinámicamente)

        # -- Contenedor para DRY RUN (Modo Prueba) --
        self.f_dryrun_outlook = tk.Frame(frame, bg="#FFF3CD", padx=10, pady=10)
        self.f_dryrun_outlook.pack(fill=tk.X, pady=10)
        self.var_dryrun = tk.BooleanVar(value=True)
        chk_dry = tk.Checkbutton(self.f_dryrun_outlook, text="MODO PRUEBA (Solo abrir borradores en Outlook)", variable=self.var_dryrun, 
                                 bg="#FFF3CD", font=("Segoe UI", 10, "bold"), activebackground="#FFF3CD")
        chk_dry.pack(anchor="w")
        
        # -- Botón de Acción --
        self.btn_enviar = ttk.Button(frame, text="📤 ENVIAR CORREOS", style="Accent.TButton", command=self.ejecutar_envio)
        self.btn_enviar.pack(fill=tk.X, pady=20, ipady=8)
        
        # Inicializar la UI
        self.actualizar_ui_envio()
    ...
    def ejecutar_generacion(self):
        ...
        # DATOS DE FIRMA
        datos_firma = None
        nombre_firmante = None # <--- NUEVA VARIABLE
        ...
        if self.var_firmar.get():
            ...
            # --- NUEVO: INTENTAMOS LEER EL NOMBRE DEL CERTIFICADO ---
            from utils import obtener_nombre_del_pfx
            nombre_extraido = obtener_nombre_del_pfx(pfx, pwd)
            
            if nombre_extraido:
                nombre_firmante = nombre_extraido
                self.log(f"🔑 Certificado detectado: {nombre_firmante}")
            else:
                messagebox.showwarning("Aviso", "No se pudo leer el nombre del certificado (quizás contraseña incorrecta).\nSe usará un nombre genérico.")
                nombre_firmante = "Firma Digital"
            # --------------------------------------------------------

            datos_firma = (pfx, pwd)
        
        # Pasamos el nombre_firmante al hilo
        threading.Thread(target=self._hilo_gen, args=(excel_path, out_folder, opts, datos_firma, nombre_firmante)).start()

    # Actualizamos también la función del hilo para recibir el nuevo argumento
    def _hilo_gen(self, excel, out, opts, datos_firma=None, nombre_firmante=None):
        generador.procesar_excel_y_generar(excel, out, opts, self.logo_path, self.log, datos_firma, nombre_firmante)
    ...
    def ejecutar_envio(self):
        metodo = self.var_metodo_envio.get()
        ...
        if metodo == "outlook":
            ...
            threading.Thread(target=self._hilo_envio_outlook, args=(excel, folder, dry)).start()
        else:
            # Lógica para SMTP
            user = self.ent_smtp_user.get()
            pwd = self.ent_smtp_pass.get()
            if not user or not pwd:
                messagebox.showerror("Error", "Faltan correo o contraseña.")
                return
            
            # Obtener config del proveedor
            if metodo in mailer.SMTP_PROVIDERS:
                config = mailer.SMTP_PROVIDERS[metodo]
            else: # Manual
                server = self.ent_smtp_server.get()
                port = self.ent_smtp_port.get()
                if not server or not port:
                    messagebox.showerror("Error", "Faltan datos del servidor SMTP.")
                    return
                config = {"server": server, "port": int(port), "tls": True}
            
            threading.Thread(target=self._hilo_envio_smtp, args=(config, user, pwd, excel, folder)).start()

    def _hilo_envio_smtp(self, config, user, pwd, excel, folder):
        mailer.enviar_masivo_smtp(config, user, pwd, excel, folder, self.log)
        self.root.after(0, lambda: messagebox.showinfo("Fin", "Proceso de envío SMTP terminado."))

    def _hilo_envio_outlook(self, excel, folder, dry):
    ...
    def actualizar_ui_envio(self):
        # Limpiar campos SMTP anteriores
        for widget in self.f_smtp_fields.winfo_children():
            widget.destroy()
            
        metodo = self.var_metodo_envio.get()
        
        if metodo == "outlook":
            self.f_smtp_fields.pack_forget() # Ocultar campos SMTP
            self.f_dryrun_outlook.pack(fill=tk.X, pady=10) # Mostrar opción Dry Run
        else:
            self.f_dryrun_outlook.pack_forget() # Ocultar opción Dry Run (no aplica a SMTP)
            self.f_smtp_fields.pack(fill=tk.X, pady=10) # Mostrar campos SMTP

            # --- Rellenar campos SMTP ---
            ttk.Label(self.f_smtp_fields, text="Tu dirección de correo (Remitente):").pack(anchor="w")
            self.ent_smtp_user = ttk.Entry(self.f_smtp_fields)
            self.ent_smtp_user.pack(fill=tk.X, pady=(2, 10))
            
            ttk.Label(self.f_smtp_fields, text="Contraseña (o Contraseña de Aplicación si usas 2FA):").pack(anchor="w")
            self.ent_smtp_pass = ttk.Entry(self.f_smtp_fields, show="*")
            self.ent_smtp_pass.pack(fill=tk.X, pady=(2, 10))
            
            # Si es manual, dejamos que el usuario edite servidor/puerto
            if metodo == "manual":
                ttk.Label(self.f_smtp_fields, text="Servidor SMTP:").pack(anchor="w")
                self.ent_smtp_server = ttk.Entry(self.f_smtp_fields)
                self.ent_smtp_server.pack(fill=tk.X, pady=(2, 10))
                
                ttk.Label(self.f_smtp_fields, text="Puerto:").pack(anchor="w")
                self.ent_smtp_port = ttk.Entry(self.f_smtp_fields)
                self.ent_smtp_port.insert(0, "587")
                self.ent_smtp_port.pack(fill=tk.X, pady=(2, 10))
